...
export default class ShowdownBlitzPoker {
  ...
  processMove(state, move) {
    ...
    // Store salt for seed generation
    if (!state.salts) state.salts = {};
    state.salts[player] = move.salt;

    // Resolve Round if both hands are present
    if (state.discards[state.playerA] !== undefined && state.discards[state.playerB] !== undefined) {
      // BLIND DECK: Use matchId + round + Both Salts as the global seed.
      // Replacement cards cannot be predicted until both reveal their salts.
      const saltA = state.salts[state.playerA] || "";
      const saltB = state.salts[state.playerB] || "";
      const deck = this.generateDeck(state.matchId + "_" + move.round + "_" + saltA + "_" + saltB);
      
      // Player A gets cards 0-4, Player B gets cards 5-9
      const initialHandA = deck.slice(0, 5);
      const initialHandB = deck.slice(5, 10);
      
      // Calculate final hands using separate replacement pools
      const replacementsA = deck.slice(10, 15);
      const replacementsB = deck.slice(15, 20);
      
      let finalHandA = [...initialHandA];
      state.discards[state.playerA].forEach((idx, i) => { if (idx >= 0 && idx < 5) finalHandA[idx] = replacementsA[i]; });
      
      let finalHandB = [...initialHandB];
      state.discards[state.playerB].forEach((idx, i) => { if (idx >= 0 && idx < 5) finalHandB[idx] = replacementsB[i]; });

      state.hands[state.playerA] = finalHandA;
      state.hands[state.playerB] = finalHandB;
