...
export class Watcher {
  ...
  private async processMatch(onChainMatchId: bigint, escrowAddress: `0x${string}`, registryAddress: `0x${string}`) {
    ...
    try {
      ...
      const { context, moves } = await this.getSyncedMoves(dbMatchId);

      if (moves.length < 2) {
        logger.warn({ matchId: onChainMatchId.toString(), round: currentRound, movesFound: moves.length }, 'DB_NOT_SYNCED // Moves missing after both revealed on-chain');
